...
registerCmd('menu', async (conn, m, args, ctx) => {
  ...
  const sections = {
    ...
    '⚙️ OWNER': ['setprefix','setbotname','setmenuimg','setfonts','fonts','addprem','delprem','public','self','ban','unban','autoviewstatus','autolikestatus','block','setaway','deleteaway','setmsgcode','deletemsgcode'],
  };
...
registerCmd('play', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const dlResponse = await httpsGet(`https://bera-api-tech.replit.app/api/download/ytmp3?url=https://youtu.be/${videoId}`);
    const dlData = JSON.parse(dlResponse);

    // FIX: Check if the result exists before accessing download_url
    if (!dlData.result || !dlData.result.download_url) {
        return fReply(conn, m, `❌ API Error: Download link not found.`, ctx.sessionName);
    }

    const audioUrl = dlData.result.download_url;

    // DOWNLOAD TO BUFFER (Stops the 0:00 bug)
    ...
    console.log(e); // This shows the error in your Termux
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
});
registerCmd('video', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const dlResponse = await httpsGet(`https://api.fabdl.com/youtube/mp4?url=https://youtube.com/watch?v=${videoId}`);
    const dlData = JSON.parse(dlResponse);
...
registerCmd('ytmp3', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '🎵 _Processing audio..._', ctx.sessionName);
  try {
    const videoId = url.match(/(?:v=|youtu\.be\/)([^&\s]+)/)?.[1];
    if (!videoId) return fReply(conn, m, '❌ Invalid URL.', ctx.sessionName);
    
    const dlResponse = await httpsGet(`https://bera-api-tech.replit.app/api/download/ytmp3?url=https://youtu.be/${videoId}&quality=128kbps`);
    const dlData = JSON.parse(dlResponse);
    const audioUrl = dlData.result?.download_url;

    if (!audioUrl) throw new Error("Audio download link not found.");
...
registerCmd('ytmp4', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '🎬 _Processing video..._', ctx.sessionName);
  try {
    const videoId = url.match(/(?:v=|youtu\.be\/)([^&\s]+)/)?.[1];
    if (!videoId) return fReply(conn, m, '❌ Invalid URL.', ctx.sessionName);
    
    // Using Fabdl for the direct download link
    const dlResponse = await httpsGet(`https://api.fabdl.com/youtube/mp4?url=https://youtube.com/watch?v=${videoId}`);
    const dlData = JSON.parse(dlResponse);
    const videoUrl = dlData.result?.download_url || dlData.url;
    ...
    await conn.sendMessage(m.key.remoteJid, { 
      video: videoBuffer, 
      mimetype: 'video/mp4', 
      caption: '🎬 Downloaded via NOVA-XMD' 
    }, { quoted: m });
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
...
registerCmd('ig', async (conn, m, args, ctx) => {
  ...
  if (!url || !url.includes('instagram.com')) {
    return fReply(conn, m, `Usage: ${ctx.prefix}ig <instagram url>`, ctx.sessionName);
  }

  await fReply(conn, m, '📥 _Processing Instagram media..._', ctx.sessionName);

  try {
    // Using a reliable public downloader API
    const apiUrl = `https://api.agatz.xyz/api/instagram?url=${encodeURIComponent(url)}`;
    const { data: res } = await axios.get(apiUrl);

    if (res.status !== 200 || !res.data || res.data.length === 0) {
      throw new Error("API could not find media for this link.");
    }

    // Get the first media item (usually the video or image)
    const mediaUrl = res.data[0].url;
    const isVideo = res.data[0].type === 'video';

    // Download to buffer first to ensure file integrity
...
registerCmd('fb', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '📥 _Processing Facebook video..._', ctx.sessionName);
  
  try {
    const apiUrl = `https://api.agatz.xyz/api/facebook?url=${encodeURIComponent(url)}`;
    const { data: res } = await axios.get(apiUrl);

    if (!res.data || !res.data.sd) throw new Error("Could not find a download link.");

    // Use the SD link for faster mobile delivery, or HD if available
    const videoUrl = res.data.hd || res.data.sd;

    // Download to buffer to prevent 0:00 error
...
registerCmd('tiktok', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '📥 _Processing TikTok..._', ctx.sessionName);
  
  try {
    const apiUrl = `https://api.agatz.xyz/api/tiktok?url=${encodeURIComponent(url)}`;
    const { data: res } = await axios.get(apiUrl);

    if (!res.data || !res.data.data) throw new Error("Could not find video data.");

    // Select the no-watermark link
    const videoUrl = res.data.data.find(v => v.type === 'nowatermark')?.url || res.data.data[0].url;
    ...
    await conn.sendMessage(m.key.remoteJid, { 
      video: buffer, 
      mimetype: 'video/mp4', 
      caption: `📥 *TikTok Downloader*\n\n👤 ${res.data.author.nickname || 'Unknown'}` 
    }, { quoted: m });
...
registerCmd('upload', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const response = await new Promise((resolve, reject) => {
      ...
      const header = `--${boundary}\r\nContent-Disposition: form-data; name="reqtype"\r\n\r\nfileupload\r\n--${boundary}\r\nContent-Disposition: form-data; name="fileToUpload"; filename="upload.jpg"\r\nContent-Type: image/jpeg\r\n\r\n`;
      const footer = `\r\n--${boundary}--\r\n`;
...
registerCmd('block', async (conn, m, args, ctx) => {
  ...
  if (targetJid.split('@')[0] === botNumber)
    return fReply(conn, m, '😂 Silly, you can\'t lock yourself out!', sessionName);
  try {
    await conn.updateBlockStatus(targetJid, 'block');
    await fReply(conn, m, `✅ Blocked *${targetJid.split('@')[0]}*`, sessionName);
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, sessionName); }
});

registerCmd('setaway', async (conn, m, args, ctx) => {
  if (!ctx.isOwner) return fReply(conn, m, '❌ Owner only.', ctx.sessionName);
  const msg = args.join(' ');
  if (!msg) return fReply(conn, m, `Usage: ${ctx.prefix}setaway <your away message>`, ctx.sessionName);
  saveAwaySettings({ enabled: true, message: msg });
  await fReply(conn, m, `✅ Away message set!\n\n_"${msg}"_\n\nBot will auto-reply when someone messages you.`, ctx.sessionName);
});
...
registerCmd('setdata', async (conn, m, args, ctx) => {
  ...
  global._dataPending[ctx.sessionName] = { provider, waitingFor: m.key.id };
  await fReply(conn, m, `📦 *Setting up .${provider}*\n\nReply to THIS message with your packages & payment info.`, ctx.sessionName);
});
...
async function handleMessage(conn, m, sessionName, ownerNumber) {
  try {
    ...
    if (!msgContent.startsWith(prefix)) {
      ...
      if (global._dataPending && global._dataPending[sessionName]) {
        const pending = global._dataPending[sessionName];
        const quotedId = m.message?.extendedTextMessage?.contextInfo?.stanzaId;
        if (quotedId && quotedId === pending.waitingFor) {
          const packs = getDataPacks();
          packs[pending.provider] = msgContent;
          saveDataPacks(packs);
          ...
          await conn.sendMessage(remoteJid, {
            text: `✅ *.${providerName}* is now live!\nUsers can type ${prefix}${providerName} to get your packages.`
          }, { quoted: m });
          return;
        }
      }
