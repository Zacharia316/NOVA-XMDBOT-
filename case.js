...
registerCmd('tiktok', async (conn, m, args, ctx) => {
  ...
  const tryTikwm = async () => {
    ...
    const dlUrl = data.play || data.wmplay;
    if (!dlUrl) throw new Error('No video URL');
    return { dlUrl, title: data.title || 'TikTok Video' };
  };

  const tryApiskeith = async () => {
    ...
    const parsed = response.data;
    if (!parsed.status || !parsed.result) throw new Error('No result');
    return { dlUrl: parsed.result, title: 'TikTok Video' };
  };

  const retryWithDelay = async (fn, retries = 3, delay = 3000) => {
  ...
  try {
    let result;
    try {
      result = await retryWithDelay(tryTikwm);
    } catch {
      result = await retryWithDelay(tryApiskeith);
    }
    await conn.sendMessage(m.key.remoteJid, { video: { url: result.dlUrl }, caption: `📥 *${result.title}*\n\n_Downloaded via NOVA-XMD_` }, { quoted: m });
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName); }
});
