...
registerCmd('video', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const tryApiskeith = async (endpoint) => {
      const res = await axios.get(`https://apiskeith.top/download/${endpoint}?url=${encodeURIComponent(ytUrl)}`, {
        headers: spoofHeaders,
        timeout: 30000
      });
      const dlUrl = typeof res.data?.result === 'string' ? res.data.result
        : res.data?.result?.url || res.data?.url || res.data?.link;
      if (!dlUrl) throw new Error('No URL');
      return dlUrl;
    };

    const tryOceansaver = async () => {
      const initRes = await axios.get(`https://p.oceansaver.in/ajax/download.php?format=mp4&url=${encodeURIComponent(ytUrl)}&api=`, {
        timeout: 30000
      });
      const { id, progress_url } = initRes.data;
      if (!id || !progress_url) throw new Error('No job ID');
      for (let i = 0; i < 20; i++) {
        await new Promise(r => setTimeout(r, 3000));
        const poll = await axios.get(progress_url, { timeout: 15000 });
        const dlUrl = poll.data?.download_url || poll.data?.url;
        if (dlUrl) return dlUrl;
      }
      throw new Error('Oceansaver timeout');
    };

    let dlUrl = null;
    const endpoints = ['mp4', 'dlmp4', 'ytmp4', 'video'];
    for (const ep of endpoints) {
      try { dlUrl = await tryApiskeith(ep); break; } catch {}
    }
    if (!dlUrl) {
      try { dlUrl = await tryOceansaver(); } catch {}
    }
    if (!dlUrl) return fReply(conn, m, '❌ All sources failed. Try again later.', ctx.sessionName);
...
registerCmd('ytmp4', async (conn, m, args, ctx) => {
  ...
  const tryApiskeith = async (endpoint) => {
    const res = await axios.get(`https://apiskeith.top/download/${endpoint}?url=${encodeURIComponent(url)}`, {
      headers: spoofHeaders,
      timeout: 30000
    });
    const dlUrl = typeof res.data?.result === 'string' ? res.data.result
  ...
  const tryOceansaver = async () => {
    const initRes = await axios.get(`https://p.oceansaver.in/ajax/download.php?format=mp4&url=${encodeURIComponent(url)}&api=`, {
      timeout: 30000
    });
    const { id, progress_url } = initRes.data;
    if (!id || !progress_url) throw new Error('No job ID');
...
registerCmd('spotify', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const dlUrl = typeof parsed.result === 'string' ? parsed.result : parsed.result?.url || parsed.result?.audio;
    if (!dlUrl) throw new Error('No result');
    await conn.sendMessage(m.key.remoteJid, { audio: { url: dlUrl }, mimetype: 'audio/mpeg', ptt: false }, { quoted: m });
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName); }
});
...
registerCmd('apk', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, `🔍 _Searching for_ *${query}*...`, ctx.sessionName);
  try {
    const response = await axios.get(`https://apiskeith.top/search/aptoide?q=${encodeURIComponent(query)}`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'Accept': 'application/json' },
      timeout: 30000
    });
    const parsed = response.data;
    if (!parsed.status || !parsed.result?.datalist?.list?.length) throw new Error('No result');
    const app = parsed.result.datalist.list[0];
    const sizeMB = app.size ? (app.size / 1024 / 1024).toFixed(2) + ' MB' : 'N/A';
    const rating = app.stats?.rating?.avg || 'N/A';
    const dlUrl = `https://app.aptoide.com/installer/${app.uname}`;
    await fReply(conn, m, `📱 *${app.name}*\n👨‍💻 *Dev:* ${app.developer?.name || 'N/A'}\n📦 *Size:* ${sizeMB}\n⭐ *Rating:* ${rating}\n📅 *Updated:* ${app.updated?.split(' ')[0] || 'N/A'}\n\n🔗 ${dlUrl}`, ctx.sessionName);
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName); }
});
...
registerCmd('mediafire', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const { fileName, fileType, size, dl_link } = parsed.result;
    await fReply(conn, m, `📦 *MediaFire Download*\n\n📄 *File:* ${fileName}\n📁 *Type:* ${fileType}\n💾 *Size:* ${size}\n\n🔗 ${dl_link}`, ctx.sessionName);
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName); }
});

// ========== FIXED TIKTOK COMMAND WITH MULTIPLE APIs ==========
registerCmd('tiktok', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '📥 _Downloading TikTok..._', ctx.sessionName);
  
  // API List - tries each until one works
  const apis = [
    {
      name: 'agatz',
      ...
      handler: (data) => {
        if (data.data && data.data.data) {
          const videoData = data.data.data.find(v => v.type === 'nowatermark') || data.data.data[0];
          return { url: videoData?.url, author: data.data.author?.nickname };
        }
        return null;
      }
    },
    {
      ...
      handler: (data) => {
        if (data.code === 0 && data.data) {
          return { 
            url: data.data.play || data.data.hdplay || data.data.wmplay,
    ...
    {
      name: 'tikcdn',
      url: `https://tikcdn.io/api/download?url=${encodeURIComponent(url)}`,
      handler: (data) => {
        if (data.status === 'success' && data.video) {
          return { url: data.video, author: data.author };
        }
        return null;
      }
    }
  ...
  // If all APIs fail
  await fReply(conn, m, `❌ Failed to download TikTok. The link may be private, deleted, or from an unsupported region.`, ctx.sessionName);
});

// ========== WORKING STICKER COMMAND ==========
registerCmd('sticker', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '🎨 _Creating sticker..._', ctx.sessionName);
  
  try {
    // Direct buffer approach - most reliable
    const buffer = await downloadMedia(mediaSource, mediaType);
...
registerCmd('toimg', async (conn, m, args, ctx) => {
  ...
  await fReply(conn, m, '🖼️ _Converting sticker..._', ctx.sessionName);
  
  try {
    // Download sticker as buffer
    const buffer = await downloadMedia(stickerObj, 'sticker');
...
registerCmd('tts', async (conn, m, args, ctx) => {
  ...
  if (!lang || !text) return fReply(conn, m, `Usage: ${ctx.prefix}tts <lang> <text>\nExample: ${ctx.prefix}tts en Hello world`, ctx.sessionName);
  try {
...
registerCmd('voicemenu', async (conn, m, args, ctx) => {
  await fReply(conn, m,
    `🎙️ *Voice Menu*\n\n` +
    `◈ *${ctx.prefix}tts <lang> <text>*\n_Speak in any language_\nExample: ${ctx.prefix}tts sw Habari yako\n\n` +
    `◈ *${ctx.prefix}say <text>*\n_Quick English voice_\nExample: ${ctx.prefix}say Hello world\n\n` +
    `🌍 *Common lang codes:*\n` +
    `en • sw • fr • es • ar • hi • pt • de`,
    ctx.sessionName
  );
});
...
registerCmd('upload', async (conn, m, args, ctx) => {
  try {
    ...
    // Identify the media (must be a reply or include media)
    ...
    if (!mediaMsg) return fReply(conn, m, '❗ Reply to an image/video/audio/document/sticker to upload and get a Catbox URL.', ctx.sessionName);

    await fReply(conn, m, '📤 _Downloading media..._', ctx.sessionName);

    // Download media into buffer
    const mediaType = (mediaMsg.mimetype || '').split('/')[0] || 'file';
    const stream = await downloadContentFromMessage(mediaMsg, mediaType).catch(e => { 
    ...
    await fReply(conn, m, '📤 _Uploading to Catbox..._', ctx.sessionName);
    ...
    // Try interactive message (Buttons)
    try {
            ...
            interactiveMessage: {
              ...
              nativeFlowMessage: {
                buttons: [
                  ...
                  {
                    name: "cta_open",
                    buttonParamsJson: `{"display_text":"Open Link","open_url":"${catboxUrl}"}`
                  }
                ]
              }
            }
      ...
      const waMsg = generateWAMessageFromContent(m.key.remoteJid, content, { quoted: m });
      await conn.relayMessage(m.key.remoteJid, waMsg.message, { messageId: waMsg.key.id });
      
    } catch (e) {
      // Fallback if interactive buttons fail
      await conn.sendMessage(m.key.remoteJid, { text: caption }, { quoted: m });
    }

  } catch (err) {
    console.error('tourl error:', err);
    await fReply(conn, m, `❌ Failed: ${err.message}`, ctx.sessionName);
  }
});
...
registerCmd('block', async (conn, m, args, ctx) => {
  ...
  if (!ctx.isOwner) return fReply(conn, m, '❌ This command is for the Owner only.', ctx.sessionName);

  // Identify target: Mention, Quoted Sender, or raw number in args
  let target = (m.mentionedJid && m.mentionedJid[0]) || (m.quoted && m.quoted.sender) || args[0];
  
  if (!target) return fReply(conn, m, `Usage: ${ctx.prefix}block @user or ${ctx.prefix}block <number>`, ctx.sessionName);

  // Normalize number if provided as plain text
  if (typeof target === 'string' && !target.includes('@')) {
    target = target.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
  }

  try {
    // Check for various Baileys block methods to prevent crashes
    if (typeof conn.updateBlockStatus === 'function') {
      ...
      await conn.updateBlock(target, 'block');
    } else {
      return fReply(conn, m, '❌ Block API not supported by this Baileys version.', ctx.sessionName);
    }

    fReply(conn, m, `✅ Successfully blocked: @${target.split('@')[0]}`, ctx.sessionName);
  } catch (e) {
    console.error('Block error:', e);
    fReply(conn, m, `❌ Failed to block: ${e.message}`, ctx.sessionName);
  }
});
