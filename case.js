...
registerCmd('menu', async (conn, m, args, ctx) => {
  ...
  const sections = {
    ...
    '🤖 AI': ['ai','ask','gpt','imagine','codeai','advice','chatboton','chatbotoff'],
    '🎭 FUN': ['joke','fact','quote','compliment','dare','truth','roast','8ball'],
    '🎵 MUSIC': ['play','video','lyrics','spotify'],
    '📥 DOWNLOADS': ['ig','fb','tiktok','ytmp3','ytmp4','mediafire'],
    ...
    '👤 PROFILE': ['getpp','bio','id'],
    '🎮 GAMES': ['rps','guess','trivia'],
    '👥 GROUP': ['tagall','groupinfo','promote','demote','kick','add','mute','unmute','group','antilink','antidelete'],
    '🎬 MOVIES': ['movie'],
    '⚙️ OWNER': ['setprefix','setbotname','setmenuimg','setfonts','fonts','addprem','delprem','public','self','ban','unban','autoviewstatus','autolikestatus'],
  };
...
const clean = response.replace(/^(<html|Moved|Redirecting)[^\n]*/i, '').trim();
await fReply(conn, m, `🤖 *NOVA AI*\n\n${clean || '❌ No response.'}`, ctx.sessionName);
  } catch (e) {
...
registerCmd('play', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const dlData = JSON.parse(await httpsGet(`https://api.fabdl.com/youtube/mp3?url=https://youtube.com/watch?v=${videoId}`));
    const audioUrl = dlData.result?.download_url || dlData.url;
    await conn.sendMessage(m.key.remoteJid, {
      image: { url: thumbnail },
      caption: `🎵 *${title}*\n\n🔗 youtu.be/${videoId}\n\n_Downloading..._`
    }, { quoted: m });
...
registerCmd('video', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const title = titleMatch ? titleMatch[1] : query;
    const thumbnail = `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`;
    const dlData = JSON.parse(await httpsGet(`https://api.fabdl.com/youtube/mp4?url=https://youtube.com/watch?v=${videoId}`));
    const videoUrl = dlData.result?.download_url || dlData.url;
    await conn.sendMessage(m.key.remoteJid, {
      image: { url: thumbnail },
      caption: `🎬 *${title}*\n\n🔗 youtu.be/${videoId}\n\n_Downloading..._`
    }, { quoted: m });
...
registerCmd('ytmp3', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url || !url.includes('youtu')) return fReply(conn, m, `Usage: ${ctx.prefix}ytmp3 <youtube url>`, ctx.sessionName);
  await fReply(conn, m, '🎵 _Downloading audio..._', ctx.sessionName);
  try {
    const videoId = url.match(/(?:v=|youtu\.be\/)([^&\s]+)/)?.[1];
    if (!videoId) return fReply(conn, m, '❌ Invalid URL.', ctx.sessionName);
    const dlData = JSON.parse(await httpsGet(`https://api.fabdl.com/youtube/mp3?url=https://youtube.com/watch?v=${videoId}`));
    const audioUrl = dlData.result?.download_url || dlData.url;
    await conn.sendMessage(m.key.remoteJid, { audio: { url: audioUrl }, mimetype: 'audio/mpeg', ptt: false }, { quoted: m });
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
});
...
registerCmd('ytmp4', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url || !url.includes('youtu')) return fReply(conn, m, `Usage: ${ctx.prefix}ytmp4 <youtube url>`, ctx.sessionName);
  await fReply(conn, m, '🎬 _Downloading video..._', ctx.sessionName);
  try {
    const videoId = url.match(/(?:v=|youtu\.be\/)([^&\s]+)/)?.[1];
    if (!videoId) return fReply(conn, m, '❌ Invalid URL.', ctx.sessionName);
    const videoUrl = `https://api.fabdl.com/youtube/mp4?url=https://youtube.com/watch?v=${videoId}`;
    await conn.sendMessage(m.key.remoteJid, { video: { url: videoUrl }, mimetype: 'video/mp4', caption: '🎬 Downloaded' }, { quoted: m });
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
});

registerCmd('lyrics', async (conn, m, args, ctx) => {
  const query = args.join(' ');
  if (!query) return fReply(conn, m, `Usage: ${ctx.prefix}lyrics <song name>`, ctx.sessionName);
  await fReply(conn, m, '🎵 _Searching lyrics..._', ctx.sessionName);
  try {
    const data = await httpsGet(`https://api.lyrics.ovh/suggest/${encodeURIComponent(query)}`);
    const parsed = JSON.parse(data);
    if (!parsed.data?.length) return fReply(conn, m, '❌ No results.', ctx.sessionName);
    const song = parsed.data[0];
    const lyricsData = await httpsGet(`https://api.lyrics.ovh/v1/${encodeURIComponent(song.artist.name)}/${encodeURIComponent(song.title)}`);
    const lyricsObj = JSON.parse(lyricsData);
    const lyrics = lyricsObj.lyrics?.substring(0, 3000) || 'No lyrics found.';
    await fReply(conn, m, `🎵 *${song.title}* — ${song.artist.name}\n\n${lyrics}`, ctx.sessionName);
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
});
...
registerCmd('spotify', async (conn, m, args, ctx) => {
  const query = args.join(' ');
  if (!query) return fReply(conn, m, `Usage: ${ctx.prefix}spotify <song name>`, ctx.sessionName);
  try {
    const data = await httpsGet(`https://api.lyrics.ovh/suggest/${encodeURIComponent(query)}`);
    const parsed = JSON.parse(data);
    if (!parsed.data?.length) return fReply(conn, m, '❌ No results.', ctx.sessionName);
    const song = parsed.data[0];
    await conn.sendMessage(m.key.remoteJid, {
      image: { url: song.album?.cover_medium || 'https://i.imgur.com/placeholder.jpg' },
...
registerCmd('ig', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url || !url.includes('instagram')) return fReply(conn, m, `Usage: ${ctx.prefix}ig <instagram url>`, ctx.sessionName);
  await fReply(conn, m, '📥 _Downloading from Instagram..._', ctx.sessionName);
  try {
    const apiUrl = `https://api.cobalt.tools/api/json`;
    const response = await new Promise((resolve, reject) => {
      const data = JSON.stringify({ url, vCodec: 'h264', vQuality: '720', aFormat: 'mp3', isAudioOnly: false });
      const options = {
    ...
    if (parsed.url) {
      await conn.sendMessage(m.key.remoteJid, { video: { url: parsed.url }, mimetype: 'video/mp4', caption: '📥 Downloaded from Instagram' }, { quoted: m });
    } else {
      await fReply(conn, m, '❌ Could not download. Try another link.', ctx.sessionName);
    }
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
...
registerCmd('fb', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url || !url.includes('facebook')) return fReply(conn, m, `Usage: ${ctx.prefix}fb <facebook url>`, ctx.sessionName);
  await fReply(conn, m, '📥 _Downloading from Facebook..._', ctx.sessionName);
  try {
    const response = await new Promise((resolve, reject) => {
      const data = JSON.stringify({ url, vCodec: 'h264', vQuality: '720', aFormat: 'mp3', isAudioOnly: false });
      const options = { hostname: 'api.cobalt.tools', path: '/api/json', method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' } };
    ...
    const parsed = JSON.parse(response);
    if (parsed.url) {
      await conn.sendMessage(m.key.remoteJid, { video: { url: parsed.url }, mimetype: 'video/mp4', caption: '📥 Downloaded from Facebook' }, { quoted: m });
    } else {
      await fReply(conn, m, '❌ Could not download.', ctx.sessionName);
    }
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
...
registerCmd('tiktok', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url || !url.includes('tiktok')) return fReply(conn, m, `Usage: ${ctx.prefix}tiktok <tiktok url>`, ctx.sessionName);
  await fReply(conn, m, '📥 _Downloading from TikTok..._', ctx.sessionName);
  try {
    const response = await new Promise((resolve, reject) => {
      const data = JSON.stringify({ url, vCodec: 'h264', vQuality: '720', aFormat: 'mp3', isAudioOnly: false });
      const options = { hostname: 'api.cobalt.tools', path: '/api/json', method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' } };
    ...
    const parsed = JSON.parse(response);
    if (parsed.url) {
      await conn.sendMessage(m.key.remoteJid, { video: { url: parsed.url }, mimetype: 'video/mp4', caption: '📥 Downloaded from TikTok' }, { quoted: m });
    } else {
      await fReply(conn, m, '❌ Could not download.', ctx.sessionName);
    }
  } catch (e) {
...
registerCmd('mediafire', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url || !url.includes('mediafire')) return fReply(conn, m, `Usage: ${ctx.prefix}mediafire <mediafire url>`, ctx.sessionName);
  await fReply(conn, m, '📥 _Fetching MediaFire link..._', ctx.sessionName);
  try {
    const html = await httpsGet(url);
    const match = html.match(/href="(https:\/\/download[^"]+)"/);
    if (!match) return fReply(conn, m, '❌ Could not extract download link.', ctx.sessionName);
    await fReply(conn, m, `📥 *MediaFire Download*\n\n🔗 ${match[1]}`, ctx.sessionName);
  } catch (e) {
    await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName);
  }
});
...
registerCmd('tts', async (conn, m, args, ctx) => {
  ...
  try {
    const url = `https://api.voicerss.org/?key=free&hl=en-us&src=${encodeURIComponent(text)}&c=MP3&f=16khz_16bit_stereo`;
...
registerCmd('news', async (conn, m, args, ctx) => {
  await fReply(conn, m, '📰 _Fetching news..._', ctx.sessionName);
  try {
    const data = await httpsGet('https://api.currentsapi.services/v1/latest-news?language=en&apiKey=free');
    const parsed = JSON.parse(data);
    const articles = parsed.news?.slice(0, 5) || [];
    if (!articles.length) return fReply(conn, m, '❌ No news found.', ctx.sessionName);
    let text = '📰 *Latest News*\n\n';
    articles.forEach((a, i) => { text += `*${i+1}. ${a.title}*\n${a.url}\n\n`; });
    await fReply(conn, m, text, ctx.sessionName);
  } catch {
    await fReply(conn, m, `📰 Latest news: https://news.google.com`, ctx.sessionName);
  }
});

registerCmd('screenshot', async (conn, m, args, ctx) => {
  const url = args[0];
  if (!url) return fReply(conn, m, `Usage: ${ctx.prefix}screenshot <url>`, ctx.sessionName);
  await fReply(conn, m, '📸 _Taking screenshot..._', ctx.sessionName);
  try {
    const imgUrl = `https://api.apiflash.com/v1/urltoimage?access_key=free&url=${encodeURIComponent(url)}&width=1280&height=720`;
    await conn.sendMessage(m.key.remoteJid, { image: { url: imgUrl }, caption: `📸 Screenshot of: ${url}` }, { quoted: m });
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName); }
});
...
registerCmd('upload', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const response = await new Promise((resolve, reject) => {
      ...
      const options = { hostname: 'api.imgbb.com', path: `/1/upload?key=free`, method: 'POST', headers: { 'Content-Type': 'application/json' } };
      const req = https.request(options, res => { let d = ''; res.on('data', chunk => d += chunk); res.on('end', () => resolve(d)); });
      req.on('error', reject);
      req.write(data);
      req.end();
    ...
    const parsed = JSON.parse(response);
    const url = parsed.data?.url || parsed.data?.display_url;
    if (url) await fReply(conn, m, `☁️ *Uploaded!*\n\n🔗 ${url}`, ctx.sessionName);
    else await fReply(conn, m, '❌ Upload failed.', ctx.sessionName);
  } catch (e) { await fReply(conn, m, `❌ Failed: ${e.message}`, ctx.sessionName); }
});
...
registerCmd('antilink', async (conn, m, args, ctx) => {
  if (!ctx.isOwner&&!ctx.isAdmin) return fReply(conn, m, '❌ Admin only.', ctx.sessionName);
  const o = (args[0]||'').toLowerCase();
  if (!['off','del','warn','kick'].includes(o)) return fReply(conn, m, `Usage: ${ctx.prefix}antilink off/del/warn/kick`, ctx.sessionName);
  setSessionSetting(ctx.sessionName, 'antilink', o);
  await fReply(conn, m, `✅ Antilink: ${o.toUpperCase()}`, ctx.sessionName);
});
...
async function handleAntilink(conn, m, sessionName, senderJid, botNumber) {
  ...
  if (mode === 'del') { await conn.sendMessage(remoteJid, { delete: m.key }); }
  else if (mode === 'warn') { await conn.sendMessage(remoteJid, { delete: m.key }); await conn.sendMessage(remoteJid, { text: `⚠️ @${senderNumber} no links!`, mentions: [senderJid] }); }
  else if (mode === 'kick') { await conn.sendMessage(remoteJid, { delete: m.key }); await conn.groupParticipantsUpdate(remoteJid, [senderJid], 'remove'); }
}
...
async function handleMessage(conn, m, sessionName, ownerNumber) {
  try {
    ...
    if (!msgContent.startsWith(prefix)) {
      ...
      if (chatbotOn) {
        try {
          const reply = await httpsGet(`https://text.pollinations.ai/${encodeURIComponent(msgContent)}`);
          await conn.sendMessage(remoteJid, { text: reply.trim() }, { quoted: m });
        } catch {}
      }
      return;
    }
