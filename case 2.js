...
registerCmd('ig', async (conn, m, args, ctx) => {
  ...
  try {
    ...
    const mediaUrl = res.data?.[0]?.url || res.result?.[0]?.url || res.url;
    if (!mediaUrl) throw new Error("No media found for this link.");

    const mediaRes = await axios.get(mediaUrl, { responseType: 'arraybuffer', timeout: 60000 });
    const buffer = Buffer.from(mediaRes.data);

    const isVideo = mediaUrl.includes('.mp4') || res.data?.[0]?.type === 'video';
...
registerCmd('fb', async (conn, m, args, ctx) => {
  ...
  try {
    // Using the new requested endpoint
    const apiUrl = `https://apis.prexzyvilla.site/download/facebook?url=${encodeURIComponent(url)}`;
    const { data: res } = await axios.get(apiUrl);

    if (!res.status || !res.result) throw new Error("Could not find a download link.");

    // The API usually provides quality options in the result object
    const videoUrl = res.result.hd || res.result.sd || res.result.url;

    if (!videoUrl) throw new Error("No valid video URL found.");

    // BUFFER FIX: Download to buffer to prevent 0:00 duration error
...
registerCmd('tiktok', async (conn, m, args, ctx) => {
  ...
  if (!text || !text.includes('tiktok.com')) {
    return fReply(conn, m, `Usage: ${ctx.prefix}tiktok <tiktok url>`, ctx.sessionName);
  }
  ...
  try {
    ...
    const response = await axios.get(
      `https://api.nexoracle.com/downloader/tiktok-wm?apikey=cf802ac56f7d63ac14&url=${encodeURIComponent(text)}`
    );

    const result = response.data.result;
    if (!result || !result.url) throw new Error('Video stream not found');

    const caption = 
      `📥 *TikTok Downloader*\n\n` +
      `📝 *Title:* ${result.title || 'Unknown'}\n` +
      `👤 *Creator:* ${result.author?.nickname || 'Unknown'}\n` +
      `⏱️ *Duration:* ${result.duration || 'N/A'}s`;

    // ─── BUFFER DOWNLOAD (Fixes 0:00 Error) ───
    // This downloads the bytes directly to your bot first
